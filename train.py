...
parser.add_argument('--distance', type=str, default='wasserstein_2', help='distance')
        ...
        for e in range(Epoch):
            ...
            if online:
                ...
                if early_stopper[task].early_stop_check(val_n_ap, sgnn, model, train_memory_backup, time_now, task, train_IB_backup, train_PGen_backup) or e == Epoch - 1:
                    ...
                    sgnn = torch.load(best_model_path, weights_only=True)
                    ...
                    for k in range(task+1):
                        ...
                        best_mem = torch.load(best_mem_path, weights_only=True)
                        sgnn.restore_memory(best_mem)
                        if model=='OTGNet':
                            if not dis_IB:
                                best_IB = torch.load(best_IB_path, weights_only=True)
                                if dataset != 'reddit' and dataset != 'yelp':
                                    sgnn.restore_IB(best_IB)
                            if uml:
                                best_PGen = torch.load(best_PGen_path, weights_only=True)
